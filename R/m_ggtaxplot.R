...
ggtaxplot = function(metabarlist, taxo, sep.level=NULL, sep.info=NULL, thresh=NULL) {
  ...
  if(suppressWarnings(check_metabarlist(metabarlist))) {
    ...
    if(length(taxo)==1){
      ...
      taxolev.dict1 = c("superkingdom", "kingdom", "subkingdom",
      ...
      path = sapply(1:nrow(metabarlist$motus),
    ...
    gp =
      ...
      geom_segment(data=edf,
                   aes(x=from.x, xend = to.x,
                       y=from.y,yend = to.y),size=0.2, colour="grey") +
      geom_point() +
    ...
    if(is.null(thresh)) {
      ...
      gp = gp + geom_text(data=vdf[which(vdf$reads>thresh),], aes(label=name2),
