...
#' @param table             the table where the information on which the subsetting is based. Can be one of `reads`, `motus`, `pcrs`, or `samples`.
#' @param indices           a boolean vector indicating the elements, i.e. rows or columns to keep. Elements to keep should be encoded \code{TRUE}.
#'
#' @name subset_metabarlist
#'
#' @return a \code{metabarlist} object similar to `metabarlist` except that it contain only the selected elements.
#'
#' @details
...
#' # (i.e. by searching for "Annelida" in MOTUs taxonomic assignements)
...
#'
#' # Create a subset of soil_euk containing only positive controls (i.e. excluding biological samples too who are NA in soil_euk$pcrs$control_type)
#' poscontrols <- subset_metabarlist(soil_euk,
#'   table = "pcrs",
#'   indices = (soil_euk$pcrs$control_type == "positive" & !is.na(soil_euk$pcrs$control_type))
#' )
#'
...
subset_metabarlist <- function(metabarlist, table, indices) {
  if (suppressWarnings(check_metabarlist(metabarlist))) {
    ...
    if (length(indices) == 0 | is.logical(indices) == F) {
