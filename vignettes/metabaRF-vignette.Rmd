...
<<<<<<< HEAD
```{r ggpcrplate, warning=F, message=F, cache=T, fig.width=7, fig.height=5, eval = F}
...
<<<<<<< HEAD
```{r ggpcrtag, warning=F, message=F, cache=T, fig.width=7, fig.height=7, eval = F}
...
```{r subset, message=F, warning=F, cache=T, eval = F}
...
```{r hillraref, message=F, warning=F, cache=T, eval = F}
...
```{r gghill, message=F, warning=F, cache=T, fig.width=7, fig.height=2, eval = F}
...
```{r gghill2, message=F, warning=F, cache=T, fig.width=7, fig.height=2.5, eval = F}
...
contaminant_PCR = contaslayer(soil_euk$reads, controls = pcr.controls)
head(soil_euk$motus[contaminant_PCR,])
```

Visualisation of where MOTUs identified as contaminants can help clarify whether their presence is widespread across samples. 

<<<<<<< HEAD
```{r contaslayer2, message=F, warning=F, cache=T, fig.width=6, fig.height=3, eval = F}
=======
...
p = ggpcrplate(soil_euk, table = "reads", index = max.conta)+
  labs(size="#reads of most abundant contaminant")+
      ggtitle("Distribution of the most abundant contaminant")
```

In this case, the most abundant contaminant seems to be most prevalent in the pcr controls, but it appears in all samples. This tool can also be used to see whether contamination occurs during sequencing by using the same approach for the extraction controls.

<<<<<<< HEAD
```{r contaslayer3, message=F, warning=F, cache=T, fig.width=6, fig.height=3, eval = F}
=======
```{r contaslayer3, message=F, warning=F, cache=T, fig.width=6, fig.height=3}
>>>>>>> master
#finds contaminants from sequencing step
pcr.controls = rownames(soil_euk$pcrs)[which(soil_euk$pcrs$control_type=="extraction")]
contaminant_Ext = contaslayer(soil_euk$reads, controls = pcr.controls)
head(soil_euk$motus[contaminant_Ext,])

#Distribution of the most abundant contaminants in the PCR plate design
max.conta.2 = contaminant_Ext[which.max(soil_euk$motus[contaminant_Ext, "count"])]

p = ggpcrplate(soil_euk, table = "reads", index = max.conta.2)+
  labs(size="#reads of most abundant contaminant")+
      ggtitle("Distribution of the most abundant contaminant")
```

Again, this OTU appears abundant across all samples, but is very high in extraction control in plate 8 (position B8).

By recording OTUs deemed contaminants, these can be removed from the subsequent dataset later.
...
not sure how to do this, this uses old format of ggpcrplate??? 

<<<<<<< HEAD
```{r tagjump, message=F, warning=F, cache=T, fig.width=6, fig.height=3, eval=F, eval = F}
...
tag.samples = rownames(soil_euk$pcrs)[which(soil_euk$pcrs$control_type=="NA")]
tag_switches = contaslayer(soil_euk$reads, controls = tag.controls)

max.tagswitches = tag_switches[which.max(soil_euk$motus[tag_switches, "count"])]

idx = which.max(soil_euk$motus$count)

max.conta.2 = contaminant_Ext[which.max(soil_euk$motus[contaminant_Ext, "count"])]

p1 = ggpcrplate(soil_euk, table = "reads", index = max.conta)
p1 = ggpcrplate(soil_euk_clean, table = "reads", index = max.conta)
...
#' nmds + labs(fill="sample type")
#'
#' #identify PCRs too close to negative controls
```
