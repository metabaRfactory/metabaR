...
```{r hillraref, message=F, warning=F, eval=F}
soil_euk_h20.raref = hill_rarefaction(soil_euk_h20, nboot = 20, nsteps = 10)
head(soil_euk_h20.raref$hill_table)
```

```{r hillraref hidden, message=F, warning=F, echo=F}
# soil_euk_h20.raref = hill_rarefaction(soil_euk_h20, nboot = 20, nsteps = 10)
# saveRDS(soil_euk_h20.raref, file = "soil_euk_h20.raref.rds")
soil_euk_h20.raref = readRDS("soil_euk_h20.raref.rds")
head(soil_euk_h20.raref$hill_table)
```
...
```{r pcrsalyer3, message=F, warning=F, eval=F, fig.width=5, fig.height=4, fig.align="center"}
...
```{r tagjump1, message=F, warning=F, eval=F, fig.width=10, fig.height=5, fig.align="center"}
...
```{r tagjump1 hidden, message=F, warning=F, echo=F, fig.width=10, fig.height=5, fig.align="center"}
# Define a vector of thresholds to test
thresholds <- c(0,1e-4,1e-3, 1e-2, 3e-2, 5e-2)
# Run the tests and stores the results in a list
# tests <- lapply(thresholds, function(x) tagjumpslayer(soil_euk,x))
# saveRDS(tests, file = "tests_tagjumps.rds")
tests = readRDS("tests_tagjumps.rds")
...
```{r raref,  message=F, warning=F, eval=F, fig.width=8, fig.height=3, fig.align="center"}
...
```{r raref hidden, message=F, warning=F, echo=F, fig.width=8, fig.height=3, fig.align="center"}
# soil_euk_agg.raref = hill_rarefaction(soil_euk_agg, nboot = 20, nsteps = 10)
# saveRDS(soil_euk_agg.raref, file = "soil_euk_agg.raref.rds")
soil_euk_agg.raref = readRDS("soil_euk_agg.raref.rds")
material <- paste(soil_euk_agg$samples$Habitat, soil_euk_agg$samples$Material)
material <- setNames(material,rownames(soil_euk_agg$samples))
